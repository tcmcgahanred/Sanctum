...
- **Decide MSRC volume** — `api.msrc.microsoft.com/update-guide/rss` produced 3,561 of 6,757 lifetime articles (52.7%, ~396/cycle) against a next-largest sensor at 270 lifetime. Per-CVE Patch Tuesday enumeration, not intelligence. Directly contradicts "quality over quantity on sensors." Options: drop, filtered variant, or per-source intake cap. **Second reason added 2026-08-25:** the feed carries 4,701 entries, 71 of them inside the cycle window, and **every one is unusable** — the item links resolve to a JavaScript shell that stores "You need to enable JavaScript to run this app" in place of an article. Measured by `tools/sensor_bench.py`; no fetch strategy recovers it. Microsoft publishes the same data through an official API (`api.msrc.microsoft.com/cvrf/v3.0/`, verified reachable), which would deliver the same flood more cleanly — **the useful move is the opposite direction: retire it as a sensor and keep it as a lookup.**
